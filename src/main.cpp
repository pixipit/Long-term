...
Adafruit_BME280 indoorSensor; // I2C
...
void init(){
  ...
  unsigned status;
  status = indoorSensor.begin(118);  
  if (!status) {
        Serial.println("Could not find a valid BME280 sensor, check wiring, address, sensor ID!");
        Serial.print("SensorID was: 0x"); Serial.println(indoorSensor.sensorID(),16);
...
void addIndoorData(WeatherData& dataBuffer){
  ...
  dataBuffer.temp = indoorSensor.readTemperature();
  dataBuffer.humidity = indoorSensor.readHumidity();
  Serial.println(dataBuffer.temp);
}
...
void run(){
  ...
  addIndoorData(data.weatherDataOffline); // adds data from sensor
...
void setup()
{
  ...
  indoorSensor.write8(BME280_REGISTER_CONTROL, indoorSensor.MODE_SLEEP);
